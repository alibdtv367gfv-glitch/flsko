  ...
  return (
    <ScreenContainer className="px-5 pt-4">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <View className="mt-5 gap-3">{visible.length ? visible.map((file) => <View key={file.id} className="flex-row items-center rounded-2xl border border-border bg-surface p-3"><Pressable onPress={() => void Linking.openURL(file.storageUrl)} className="flex-1 flex-row items-center"><View className="h-16 w-16 items-center justify-center overflow-hidden rounded-xl bg-background">{file.kind === "image" ? <Image source={{ uri: file.storageUrl }} className="h-full w-full" resizeMode="cover" /> : <Text className="text-2xl">{file.kind === "video" ? "▶" : "▤"}</Text>}</View><View className="mr-3 flex-1"><Text numberOfLines={1} className="text-sm font-bold text-foreground">{file.name}</Text><Text className="mt-1 text-xs text-muted">{file.kind === "image" ? "صورة" : file.kind === "video" ? "فيديو" : "مستند"} · {Math.ceil(file.sizeBytes / 1024)} كيلوبايت</Text></View><Text className="text-xs font-bold text-primary">فتح</Text></Pressable><Pressable onPress={() => Alert.alert("حذف الملف", "سيختفي الملف من مساحتك ولن يستخدمه الوكيل بعد الآن.", [{ text: "إلغاء", style: "cancel" }, { text: "حذف", style: "destructive", onPress: () => remove.mutate({ id: file.id }) }])} className="mr-2 rounded-full px-2 py-2"><Text className="text-xs font-bold text-error">حذف</Text></Pressable></View>) : <View className="rounded-2xl border border-dashed border-border p-8"><Text className="text-center text-sm leading-6 text-muted">لا توجد ملفات في هذا القسم بعد. ابدأ بإضافة صورة أو فيديو أو مستند.</Text></View>}</View>
        <View className="mt-6 rounded-3xl bg-surface p-4"><Text className="text-sm font-bold text-foreground">الخصوصية والتنظيف</Text><Text className="mt-2 text-xs leading-5 text-muted">لا يستخدم فلسقوا ملفًا في الإجابة إلا عند طلبك أو عند منحه إذنًا واضحًا. تُنظف مهام التوليد الفاشلة أو المهجورة فقط بعد 14 يومًا؛ أما ملفاتك وذكرياتك ونتائجك الناجحة فلا يحذفها الوكيل تلقائيًا.</Text></View>
      </ScrollView>
    </ScreenContainer>
  );
}
